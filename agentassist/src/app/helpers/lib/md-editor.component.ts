...
import { EditorUrlDialogComponent } from '../components/editor-url-dialog/editor-url-dialog.component';
...
export class CustomMarkdownEditorComponent implements ControlValueAccessor, Validator {
  ...
  insertContent(type: string, customContent?: string) {
    ...
    switch (type) {
      ...
      case 'Heading':
        initText = 'Heading';
        selectedText = `#h1 ${selectedText || initText}`;
        startSize = 4;
        break;
      case 'Refrence':
        initText = 'Refrence';
  ...
  toggleTheme(){
    if(this.isFullScreen) {
      this.editorInstance.setOptions({theme: "ace/theme/chaos"});
    } else {
      this.editorInstance.setOptions({theme: ""});
    }
  }
  ...
  copyToClipboard(text) {
    ...
    this.notificationService.notify(this.translate.instant("NOTIFY.SCRIPT_COPIED_SUCCESS"), "success");
  }
