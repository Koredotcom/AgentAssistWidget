    ...
    let subscription2 = this.handleSubjectService.connectDetailsSubject.subscribe((response: any) => {
      if (response) {
        ...
        headersVal = {
          ...
          $.ajax({
            url: `${connectionDetails.agentassisturl}/api/1.1/botmessages/chathistorytoagentassist?botId=${userBotConversationDetails.botId || connectionDetails?.botId}&userId=${userBotConversationDetails?.userId}&sessionId=${userBotConversationDetails?.sessionId}`,
            type: 'get',
            headers: headersVal,
            dataType: 'json',
            success:  (data) => {
