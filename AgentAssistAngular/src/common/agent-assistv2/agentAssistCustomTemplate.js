(function($){
	function agentAssistCustomTemplate(data,chatInitialize) {
		this.cfg = data;
	...
	agentAssistCustomTemplate.prototype.renderMessage = function (msgData) {
		var messageHtml = '';
    console.log('Ms teams template testing: ',msgData)
    if( window.krAdaptiveCards && window.krAdaptiveCards.AdaptiveCard && msgData.message[0] && msgData.message[0].cInfo && msgData.message[0].cInfo.body && msgData.message[0].cInfo.body.attachments && msgData.message[0].cInfo.body.attachments[0].content){
...
var message= {
	...
	agentAssistCustomTemplate.prototype.getColumnWidth = function (width) {
		var _self = this;
		var newWidth;
...
//Below method is for template specific events
	agentAssistCustomTemplate.prototype.templateEvents =function(ele, templateType, bindingData){
	...
	agentAssistCustomTemplate.prototype.bindEvents = function (messageHtml) {
		...
		$(messageHtml).find(".ratingMainComponent").off('click','[type*="radio"]').on('click','[type*="radio"]',function (e) {
			...
			if(_innerText==msgData.message[0].component.payload.starArrays.length){
				...
				$(".suggestionsMainComponent").remove();
				$(".kore-action-sheet").find(".ratingMainComponent").append(agentAssistCustomTemplate.prototype.suggestionComponent());
			    }else{
                $(".kore-action-sheet").find(".ratingMainComponent").append(agentAssistCustomTemplate.prototype.suggestionComponent());
				}
			}
		...
		$(messageHtml).find(".emojiComponent,.thumpsUpDownComponent,.numbersComponent").off('click','.emoji-rating,.numbers-rating').on('click','.emoji-rating,.numbers-rating',function(e){
			...
			if($(this).attr("value") < "5"){
				...
				if ($(".kore-action-sheet").find(".thumpsUpDownComponent").length) {
					$(".kore-action-sheet").find(".thumpsUpDownComponent").append(agentAssistCustomTemplate.prototype.suggestionComponent());
				} else if ($(".kore-action-sheet").find(".numbersComponent").length) {
					$(".kore-action-sheet").find(".numbersComponent").append(agentAssistCustomTemplate.prototype.suggestionComponent());
				} else {
					$(".kore-action-sheet").find(".emojiComponent").append(agentAssistCustomTemplate.prototype.suggestionComponent());
				}
	...
	<div class="submitButton"><button type="button" class="submitBtn">Submit</button></div>\
		...
		this.listViewTabs = function (e) {
			...
			var listValues = $(agentAssistCustomTemplate.prototype.getChatTemplate("actionSheetTemplate")).tmpl({
				'msgData': msgData,
		...
		$ele.off('click', '.advanced-list-wrapper .see-more-data').on("click", '.advanced-list-wrapper .see-more-data', function (e) {
			...
			if (messageData && messageData.message[0] && messageData.message[0].component && messageData.message[0].component.payload && messageData.message[0].component.payload.seeMoreAction === 'slider') {
				if ($(".list-template-sheet").length !== 0) {
					...
					messageHtml = $(agentAssistCustomTemplate.prototype.getChatTemplate("advancedListTemplate")).tmpl({
						'msgData': messageData,
						'helpers': helpers,
					});
					$(messageHtml).find(".advanced-list-wrapper .extra-info").hide();
					bottomSliderAction('show', messageHtml);
					agentAssistCustomTemplate.prototype.advancedListTemplateEvents(messageHtml, messageData);
				}
	...
	agentAssistCustomTemplate.prototype.cardTemplateEvents = function (ele, msgData) {
		chatInitialize = this.chatInitialize;
		helpers = this.helpers;
	...
	agentAssistCustomTemplate.prototype.proposeTimesTemplateBindEvents = function (ele, msgData) {
		if (this.chatInitialize) {
			chatInitialize = this.chatInitialize;
		}
		if(this.helpers){
		...
		$ele.off('click', '.action-buttons .propoese-element-button,.propose-template .propse-times-elements .element').on("click", '.action-buttons .propoese-element-button,.propose-template .propse-times-elements .element', function (e) {
			...
			if (type !== 'postback') {
				...
				function getproposeActionSheetTemplate() {
					...
					var proposeAtionSheetTemplate = $(agentAssistCustomTemplate.prototype.getChatTemplate("proposeActionSheetTemplate")).tmpl({
						'msgData': msgData,
	...
	agentAssistCustomTemplate.prototype.defaultCardTemplateEvents = function (ele, msgData) {
		chatInitialize = this.chatInitialize;
		helpers = this.helpers;
		var $ele = $(ele);
		var messageData = $(ele).data();
		...
		/* default card template events ends here */
			...
			/* advanced List template actions start here */
			agentAssistCustomTemplate.prototype.advancedMultiListTemplateEvents = function (ele, msgData) {
				if (this.chatInitialize) {
					chatInitialize = this.chatInitialize;
				}
				if(this.helpers){
				...
				$ele.off('click', '.advanced-multi-list-wrapper .see-more-data').on("click", '.advanced-multi-list-wrapper .see-more-data', function (e) {
					...
					if (messageData && messageData.message[0] && messageData.message[0].component && messageData.message[0].component.payload && messageData.message[0].component.payload.seeMoreAction === 'slider') {
						if ($(".list-template-sheet").length !== 0) {
							...
							messageHtml = $(agentAssistCustomTemplate.prototype.getChatTemplate("advancedMultiListTemplate")).tmpl({
								'msgData': messageData,
								'helpers': helpers,
							});
							$(messageHtml).find(".advanced-multi-list-wrapper .extra-info").hide();
							bottomSliderAction('show', messageHtml);
							agentAssistCustomTemplate.prototype.advancedMultiListTemplateEvents(messageHtml, messageData);
						}
					} else if (messageData && messageData.message[0] && messageData.message[0].component && messageData.message[0].component.payload && messageData.message[0].component.payload.seeMoreAction === 'inline') {
						if(messageData && messageData.message[0] && messageData.message[0].component && messageData.message[0].component.payload && messageData.message[0].component.payload.listViewType === 'button'){
							var hiddenElementsArray = $(ele).find('.tag-name.hide');
	...
	/* advanced multi List template actions end here */
	 ...
	 agentAssistCustomTemplate.prototype.articleTemplateEvents = function (ele, messageData) {
		if (this.chatInitialize) {
			chatInitialize = this.chatInitialize;
		}
		if (this.helpers) {
		...
		$(ele).off('click', '.article-template-content .article-show-more').on('click', '.article-template-content .article-show-more', function (e) {
			...
			if (!type || (type === 'slider')) {
				...
				var template = $(agentAssistCustomTemplate.prototype.getChatTemplate("articleTemplate")).tmpl({
					'msgData': messageData,
					'helpers': helpers,
				});
				$(template).find(".article-template .extra-info").hide();
				//$(".kore-action-sheet").append(listValues);
				agentAssistCustomTemplate.prototype.articleTemplateEvents(template, messageData)
	...
	agentAssistCustomTemplate.prototype.resetPinTemplateEvents = function (messageHtml, msgData) {
		chatInitialize = this.chatInitialize;
		helpers = this.helpers;
	...
	agentAssistCustomTemplate.prototype.otpValidationTemplateEvents = function (messageHtml, msgData) {
		chatInitialize = this.chatInitialize;
		helpers = this.helpers;
	...
	agentAssistCustomTemplate.prototype.bankingFeedbackTemplateEvents = function(messageHtml){
		var me = this;
