(function (factory) {
    ...
    window.agentAssistChat = factory();
    //}
})(function () {
    return function agentAssistChat() {
        ...
        var returnFun =(function ($,KRPerfectScrollbar) {
            ...
            var _agentAssistChat=this;
            var botMessages = {};
            var isAgentAssistEnabled;
            var _botInfo = {};
        ...
        function updateOnlineStatus() {
            ...
            window.onresize = function (event) {
                var me=AgentChatInitializes;
                if (event.target === window) {
                    AgentChatInitializes.setCollapsedModeStyles();
                    var _width = $('#chatContainer').width() - 400;
                    //$('.kore-chat-window').attr('style','left: '+_width+'+px');
                }
                if (($(`#agentAssistContainer`).width() > 400) || (document.getElementsByClassName(`#agentAssistContainer`).length && document.getElementsByClassName(`#agentAssistContainer`)[0].classList.contains('expanded'))) {
                    var _koreChatWindowHeight = $(`#agentAssistContainer`).width();
                ...
                if ($(`#agentAssistContainer`).width() > 460) {
            ...
            chatWindow.prototype.setPrivateVarToContext = function (_this) {
                ...
                _this.EVENTS=EVENTS;
                // _this.chatInitialize=AgentChatInitializes;
            ...
            chatWindow.prototype.bindEvents = function (isEventEnable,e) {
              ...
              //  });
                ...
                //     }
                // });
                if(isEventEnable && e.leftIcon){
                    e.preventDefault();
                    e.stopPropagation();
                    console.log("quick reply left icon click");
                    var _quickReplesDivs = event.currentTarget.parentElement.getElementsByClassName('buttonTmplContentChild');
                    if (_quickReplesDivs.length) {
                        var _scrollParentDiv = event.target.parentElement.getElementsByClassName('quick_replies_btn_parent');
                        if(_scrollParentDiv[0]){
                            var _totalWidth = _scrollParentDiv[0].scrollLeft;
                            var _currWidth = 0;
                            for (var i = 0; i < _quickReplesDivs.length; i++) {
                                _currWidth += (_quickReplesDivs[i].offsetWidth + 10);
                ...
                if(isEventEnable && !e.leftIcon){
                    e.preventDefault();
                    e.stopPropagation();
                    console.log("quick reply right icon click");
                    ...
                    if (_quickReplesDivs.length) {
                        ...
                        if(_scrollParentDiv[0]){
                            ...
                            for (var i = 0; i < _quickReplesDivs.length; i++) {
                                _currWidth += (_quickReplesDivs[i].offsetWidth + 10);
                                if (_currWidth > _totalWidth && _scrollParentDiv[0]) {
            ...
            chatWindow.prototype.render = function (chatWindowHtml) {
                ...
                me.setCollapsedModeStyles();
                if(me.config.agentAssist){
                    me.chatPSObj=new KRPerfectScrollbar($(`#agentAssistContainer`).get(0), {
            ...
            chatWindow.prototype.handleWebHookResponse = function (msgsData) {
                ...
                if (msgsData && msgsData.length) {
                    msgsData.forEach(function (msgData, index) {
                        setTimeout(function () {
                            AgentChatInitializes.renderMessage(msgData);
            ...
            chatWindow.prototype.renderMessage = function (msgData,elementID, runInfoContent) {
                ...
                var _chatContainer = me.config.agentAssist? $(`#agentAssistContainer`).find(`#${runInfoContent} #displayData-${elementID}`):$(me.config.chatContainer).find('.chat-container');
                ...
                if(msgData && msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.sliderView && !msgData.message[0].component.payload.fromHistory){
                    ...
                    //         var msgTimeStamps = [];
                    //         var msgEles = $(`${me.config.agentAssist?`#agentAssistContainer #${runInfoContent} #displayData-${elementID}  >li`:'.kore-chat-window .chat-container>li'}`);
                    //         if (msgEles.length) {
                    ...
                    //     }
                    // }else{
                    //     console.log($(`#agentAssistContainer #${runInfoContent} #displayData-${elementID}  li#`).length,"----<============================else=======================>came here out of else main if condition===",$(`${me.config.agentAssist?`#agentAssistContainer #displayData-${elementID}  li#`:`.kore-chat-window .chat-container li#`}` + msgData.messageId).length,msgData.messageId, msgData.renderType);
                    // }
                }
            ...
            <div class="kore-chat-window droppable liteTheme-one" id="chat-window-footer"> \
            <div class="kr-wiz-menu-chat defaultTheme-kore">\
            </div>	\
                <div class="minimized-title"></div> \
                <div class="minimized"><span class="messages"></span></div> \
                <div class="kore-chat-header" id="koreChatHeader"> \
                ...
                <div role="log" aria-live="polite" aria-atomic="true" class="kore-chat-body" > \
                    ...
                    <ul class="chat-container" id="chatContainers"></ul> \
                </div> \
                <div class="typingIndicatorContent"><div class="typingIndicator"></div><div class="movingDots"></div></div> \
                <div class="kore-chat-footer disableFooter" id ="123"> '+ chatFooterTemplate + '{{if isSendButton}}<div class="sendBtnCnt"><button class="sendButton disabled" type="button">${botMessages.sendText}</button></div>{{/if}}</div>\
                            ...
                            <div class="messageBubble" aria-live="assertive">\
                                <div> \
                                    {{if msgData.type === "bot_response"}} \
                                        ...
                                        {{else}} \
                                            {{if msgItem.component.type == "message" && msgItem.component.payload.fileUrl}} \
     ...
     var iframe = '<script id="chat_message_tmpl" type="text/x-jquery-tmpl"> \
            ...
            this.applySDKBranding = function (res) {
                AgentChatInitializes.applySDKBranding.call(AgentChatInitializes,res);
            }
            ...
            window.onbeforeunload = function () {
                if (AgentChatInitializes && $(AgentChatInitializes.config.chatContainer).length > 0) {
                    AgentChatInitializes.destroy();
                    //return null;
                }
            }
            ...
            this.addWidgetEvents = function (cfg) {
                if (cfg) {
                    ...
                    wizSDK.events.onPostback = function (data) {
                            ...
                            AgentChatInitializes.sendMessage($('.chatInputBox'), data.utterance, data);
                    };
                }
            };
            ...
            this.chatHistory = function (res) {
                AgentChatInitializes.chatHistory.call(AgentChatInitializes,res);
            }
            // chatWindow.prototype.chatHistory = function (res) {
            //     var me = this;
            //     if(res[2]==='historysync'){
            ...
            var prevStr = "";
            setTimeout(function(){
                if(AgentChatInitializes && AgentChatInitializes.config && AgentChatInitializes.config.allowGoogleSpeech) {
                    if(window.initGapi){
                        initGapi();
                    }else{
                ...
                if (rec) {
                    ...
                    intervalKey = setInterval(function () {
                        rec.export16kMono(function (blob) {
                            ...
                            if (AgentChatInitializes.config.allowGoogleSpeech) {
            ...
            function createSocket() {
                ...
                _connection.onclose = function (e) {
                    if ($('.chatInputBox').text() !== '' && AgentChatInitializes.config.autoEnableSpeechAndTTS) {
            ...
            function createSocketForTTS() {
                ...
                _ttsConnection.onmessage = function (msg) {
                    ...
                    if (typeof msg.data === 'string') {
                        ...
                        var _data = msg.data
                        if (AgentChatInitializes.isTTSOn) {
                            playsound(_data);
                        }
                    }
                };
                ...
                _ttsContext.decodeAudioData(raw, function (buffer) {
                    ...
                    try {
                        ...
                        ttsAudioSource.addEventListener('ended', function () {
                            setTimeout(function () {
                                if (AgentChatInitializes.isTTSOn && AgentChatInitializes.config.autoEnableSpeechAndTTS) {
            ...
            function getFileToken(_obj, _file, recState) {
                ...
                $.ajax({
                    ...
                    error: function (msg) {
                        AgentChatInitializes.config.botOptions._reconnecting=true;
            ...
            function getfileuploadConf(_recState) {
                ...
                _accessToke = "bearer " + AgentChatInitializes.accessToken;
                if(me.config && me.config && me.config.botOptions && me.config.botOptions.webhookConfig && me.config.botOptions.webhookConfig.enable){
            ...
            function onComponentReady(_this, data) {
                ...
                $('.chatInputBox').focus();
                AgentChatInitializes.attachmentInfo.fileName = data.values.componentData.filename;
                AgentChatInitializes.attachmentInfo.fileType = data.values.componentType;
                $('.sendButton').removeClass('disabled');
            };
            ...
            ************************************************************************************************************************************************/
            ...
            //Actual chatwindow.js agentAssistChat function code end here
