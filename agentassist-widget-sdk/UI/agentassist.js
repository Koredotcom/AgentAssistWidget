...
window.AgentAssist = function AgentAssist(containerId, _conversationId, _userId, _botId) {
    ...
    function processAgentAssistResponse(data, convId, botId, userId) {
                    ...
                    mutation.addedNodes.forEach(function(added_node) {
                      if(added_node.nodeName === 'LI'){
                        for(let nodes of added_node.children){
                            nodes.classList.remove('profile-photo');
                           
                            nodes.className.includes(['extra-info'])? nodes.remove():'';
                            
                            if(nodes.className.includes('messageBubble')){
                             if(data.type === "intent"){
                                nodes.classList.remove('messageBubble');
                                ...
                                nodes.innerHTML = iHtml;
                                
                             }
                             else{
                                 if(!parsedPayload){
                                    nodes.classList.remove('messageBubble');
                                    nodes.classList.add("btn-template-data");
                                    let btnValue;
                                    btnValue = data.buttons.shift();
                                    console.log("===== message data===",btnValue)
                                    if(btnValue){
                                        var btnHtml = `
                                                <div class="curser-pointer">"${btnValue?.value}"</div>
                                                <div class="overlay-send-copy-">
                                                    <div class="send-copy" aura:id="btn-send-${convId}-1" id="btn-send-${convId}-1" 
                                                                data-as-send="true" 
                                                                data-conv-id="${convId}" data-btn-id="1">SEND</div>
                                                    <div class="send-copy" aura:id="btn-copy-${convId}-1" id="btn-copy-${convId}-1" 
                                                                data-as-copy="true" 
                                                                data-conv-id="${convId}" data-btn-id="1">COPY</div>
                                                </div>
                                            </div>
                                        `;
                                      nodes.innerHTML = btnHtml;
                                    }
                                   
                                 }
                                 else{
                                    console.log(nodes,"when the list template comes to picture")
                                }
                              
                                
                             }
                            } else{
                                if(typeof nodes === 'object' && nodes.children.length>1){
                                    for(let node of nodes.children){
                                        if(['listTmplContentBox', 'buttonTmplContentBox','slide'].includes(node.className)){
                                            console.log(node)
                                            for(let child of node.children){
                                                if(['viewMoreList', '','carouselTitleBox'].includes(child.className)){
                                                    for(let child1 of child.children){
                                                            child1.setAttribute('data-conv-id',`${data.conversationId}`);
                                                            child1.setAttribute('data-bot-id',`${_botId}`);
                                                            child1.setAttribute('data-value',`${data.value}`);
                                                            child1.setAttribute('data-user-id',`${userId}`);
                                    ...
                                    console.log(nodes,"final mutation if condition")
                                }
      ...
      //  processData(data, data.conversationId, botId, userId, _msgsResponse);
    ...
    function btnInit() {
         ...
         _chatContainer = chatInitialize.config.chatContainer;
         _chatContainer.on('click','.buttonTmplContentBox li,.listTmplContentChild .buyBtn, .viewMoreList .viewMore,.listItemPath,.quickReply,.carouselImageContent,.listRightContent,.checkboxBtn,.likeDislikeDiv',function(e){
            AgentAssist_input_keydown(e);
         });
    ...
    function addButtons(data, _conversationId, botId, userId,resp) {
        console.log("AgentAssist >>> addingButton", data,"resp",resp);
        var mainText = data.value;
        if (!mainText && mainText.length === 0) {
            mainText = '';
        }
                    ...
                    if (btn && btn.type === 'text') {
                        ...
                        var btnHtml = `
            				<div class="btn-template-data" data-text-value="${btnValue}" data-conv-id="${convId}" 
                                ...
                                <div class="curser-pointer">"${btnValue}"</div>
            					<div class="overlay-send-copy-">
    ...
    function createAgentAssistContainer(containerId, conversationId, botId, userId) {
        ...
        console.log("AgentAssist >>> userId in createAgentAssistContainer", containerId, conversationId, userId, botId)
        var container = document.getElementById(containerId);
        var headerContainer = document.getElementById('koreChatHeader');
        var headerHtml = `<div class="title-bar">
        <div class="top-title">
            Agent Assist
        </div>
        <!-- Rounded switch -->
    ...
    var chatContainer = document.getElementById('chatContainers');
    var chatContainerHTml = `<div class="tab-content">
                <div id="botlist-${conversationId}" data-conversation-id="${conversationId}" style="overflow-y:scroll;height:100%;padding-right:10px" class="tab-pane fade active show">
            <!-- all text messages will be appended here -->
       ...
       chatContainer.innerHTML = chatContainerHTml;
        var container123 = document.getElementById('123');
        container123.classList.remove('disableFooter');
        // if(chatConfig.agentAssist){
        //     container123.style.display = "none";
        // }
        if (container123) {
            console.log("AgentAssist >>> found container", container123);
            var cHtml = `<div class="agent-with-bots">
                <div class="inner-bots-container">
                    <div class="agent-bot-types">
                        <div class="bot-list-accr">
        ...
        "message": [
            {
                "type": "text",
                "cInfo": {
                    "body": data.intentName? data.value : data.value,
                    "ignoreCheckMark": ''
                },
                "clientMessageId": data.conversationId
            }
        ],
